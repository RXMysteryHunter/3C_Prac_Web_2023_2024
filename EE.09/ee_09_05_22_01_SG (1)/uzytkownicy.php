                ...
                if(isset($_POST['login']) && isset($_POST['password'])) {
                    ...
                    if($result->num_rows == 0) {
                        ...
                        if($passwordHash == $row['haslo']) {
                            $q= "SELECT uzytkownicy.login, dane.rok_urodz, dane.przyjaciol, dane.hobby, dane.zdjecie FROM `uzytkownicy` 
                            LEFT JOIN dane ON uzytkownicy.id = dane.id WHERE login=\"$login\"";
                            $result = $db->query($q);
                            $row = $result->fetch_assoc();
                            ...
                            echo "<button>Wiecej info</button>";
                            echo "</a>";

                        } else{
                            echo "haslo nie prawidlowe";
                        }
                    }
                    }
